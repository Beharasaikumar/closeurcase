    ...
    <div className={`flex gap-2 sm:gap-3 ${className ?? ""}`} role="group" aria-label={ariaLabel}>
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          className={`h-14 w-12 sm:h-16 sm:w-14 rounded-[var(--md-sys-shape-corner-small)] border-2 bg-[var(--md-sys-color-surface)] text-center text-xl font-semibold text-[var(--md-sys-color-on-surface)] outline-none transition-colors disabled:opacity-40 ${
            error
